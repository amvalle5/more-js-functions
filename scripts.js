...
function calcChange(amount){
    // calculate the number of quarters, subtract from the change
    let quarters = Math.floor(amount / 25);
    amount = amount - (quarters * 25);

    // calculate the number of dimes, subtract from the change
    let dimes = Math.floor(amount / 10);
    amount = amount - (dimes * 10);

    // calulate the numbers of nickels, subtract from the change
    let nickels = Math.floor(amount / 5);
    amount = amount - (nickels * 5);

    // set numbers of pennies
    let pennies = amount;
